		...
		while(true)
		{
			...
			switch(ch)
			{
				case 1:
					...
					carray[noOfCustomers++] = new Customers(id, name, age, mobnum, dob, address);
					break ;
				case 2:
					...
					if(BusTicketDetails.validateTicket(ticket, carray, noOfCustomers))
				...
				case 3:
					...
					String city = sc.next();
					Customers.displayCustomersFromCity(carray, city, noOfCustomers);
					System.out.println("Enter customer Id :");
					int idC= sc.nextInt();
					Customers.displayCustomerWithId(carray, idC, noOfCustomers);
					System.out.println("Displaying customer names in sorted order :");
					Customers.sortCustomerNames(carray, noOfCustomers);
					break;
				case 4:
...
class BusTicketDetails
{
	...
	public static boolean DoesContain(Customers[] carray,int id,String mobile_number,int len)
	{
		for(int i=0;i<len;i++)
	...
	public static boolean validateTicket(BusTicketDetails ticket,Customers[] carray,int len)
	{
		boolean isValid = true;
		isValid = isValid && DoesContain(carray, ticket.id, ticket.mobile_number,len);
...
class Customers
{
	...
	public static void displayCustomersFromCity(Customers [] carray , String city,int len)
	{
		for(int i=0;i<len;i++)
			if(carray[i].address.equalsIgnoreCase(city))
				System.out.println(carray[i]);
	}
	public static void displayCustomerWithId(Customers [] carray,int id,int len)
	{
		for(int i=0;i<len;i++)
			if(carray[i].id==id)
				System.out.println(carray[i]);
	}
	public static void sortCustomerNames(Customers[] carray,int len)
	{
